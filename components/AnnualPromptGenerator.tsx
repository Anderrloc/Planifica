...
const PERIODOS = ["Bimestral", "Trimestral"];

const AnnualPromptGenerator: React.FC = () => {
  ...
  const handleUnitNameChange = (index: number, name: string) => {
    const newNames = [...data.unitNames];
    newNames[index] = name;
    setData({ ...data, unitNames: newNames });
  };

  const syncUnitNames = () => {
    const currentNames = [...data.unitNames];
    if (currentNames.length < data.numUnidades) {
      for (let i = currentNames.length; i < data.numUnidades; i++) {
        currentNames.push(`Unidad ${i + 1}`);
      }
    } else if (currentNames.length > data.numUnidades) {
      currentNames.splice(data.numUnidades);
    }
    setData({ ...data, unitNames: currentNames });
  ...
  const generatePrompt = () => {
    ...
    const unitNamesText = data.unitNames.length > 0 
      ? `\nNombres de las unidades sugeridos:\n${data.unitNames.map((name, i) => `${i + 1}. ${name}`).join("\n")}`
      : "";

    return `Actúa como un Especialista en Planificación Curricular del Ministerio de Educación (MINEDU) de Perú. 
Diseña una PROGRAMACIÓN ANUAL detallada para el año escolar ${data.año} siguiendo estrictamente esta estructura y bajo el Marco del Buen Desempeño Docente:
...
- Año Académico: ${data.año}

II. DESCRIPCIÓN GENERAL
Presenta una descripción del área y las metas para el año. Incluye un CUADRO detallado con:
- Competencias del área seleccionadas:
${compsText || "Seleccionar automáticamente según CNEB"}
- Capacidades por cada competencia.
- ESTÁNDARES DE APRENDIZAJE DE LAS COMPETENCIAS (Ciclo correspondiente).
- CRITERIOS DE EVALUACIÓN.

III. CALENDARIZACIÓN
Propón una distribución ${data.periodo}, incluyendo semanas lectivas y de gestión para el año ${data.año}.

IV. ORGANIZACIÓN DE LAS UNIDADES DE APRENDIZAJE
Presenta una matriz con EXACTAMENTE ${data.numUnidades} unidades de aprendizaje.${unitNamesText}
Cada unidad debe tener una duración de aproximadamente ${data.semanasPorUnidad} semanas.
Para cada unidad incluye:
- Número de unidad.
- Título creativo.
- Situación de contexto/Eje.
- Campo temático.
- Duración aproximada.
- Producto principal.

V. ORGANIZACIÓN DE LOS PROPÓSITOS DE APRENDIZAJE
Presenta un CUADRO que vincule:
- COMPETENCIAS SELECCIONADAS:
${compsText || "Seleccionar automáticamente según CNEB"}
- COMPETENCIAS TRANSVERSALES SELECCIONADAS:
${transCompsText || "Seleccionar automáticamente según CNEB"}
- DESEMPEÑOS precisos para el grado.
- ENFOQUES TRANSVERSALES SELECCIONADOS: ${approachesText || "Seleccionar automáticamente según CNEB"}

VI. MATERIALES Y RECURSOS EDUCATIVOS
Lista de recursos físicos y digitales alineados al CNEB.

VII. EVALUACIÓN
Describe las orientaciones para la evaluación formativa, técnicas e instrumentos sugeridos.

VIII. BIBLIOGRAFÍA
Lista de referencias bibliográficas para el docente y el estudiante (libros, guías, enlaces web, etc.).

CONTEXTO ESPECÍFICO PARA EL DISEÑO:
${data.contexto}
    ...
    <div className="space-y-8 animate-fade-in pb-20">
      <section className="bg-white rounded-[2rem] shadow-xl border border-slate-200 overflow-hidden">
        <div className="bg-slate-900 text-white px-10 py-8">
          <h2 className="text-[12px] font-black text-red-500 uppercase tracking-widest mb-2">Generador de Prompts</h2>
          <h3 className="text-2xl font-black uppercase">Programación Anual {data.año}</h3>
          <p className="text-slate-400 text-xs mt-2 font-bold uppercase tracking-widest">Configura los datos para obtener un prompt profesional</p>
        </div>
        ...
        <div className="p-10 space-y-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input 
                ...
                className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-red-600/20"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <select 
                  ...
                  className="flex-1 bg-slate-50 border border-slate-200 p-4 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-red-600/20 appearance-none"
                >
            ...
            <div className="space-y-2">
              ...
              <select 
                ...
                className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-red-600/20 appearance-none"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">N° Unidades</label>
                  <input 
                    type="number" 
                    value={data.numUnidades}
                    onChange={(e) => setData({...data, numUnidades: parseInt(e.target.value) || 0})}
                    className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-red-600/20"
                  />
                </div>
                <div className="flex items-end pb-1">
                  <button 
                    onClick={syncUnitNames}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Semanas x Unidad</label>
                <input 
                  type="number" 
                  value={data.semanasPorUnidad}
                  onChange={(e) => setData({...data, semanasPorUnidad: parseInt(e.target.value) || 0})}
                  className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-red-600/20"
                />
              </div>
            </div>
            ...
            {showUnitInputs && (
              <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-200 space-y-6 animate-fade-in">
                <div className="flex justify-between items-center">
                  <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Personalizar Nombres de Unidades</h4>
                  <button onClick={() => setShowUnitInputs(false)} className="text-red-600 text-[10px] font-black uppercase tracking-widest hover:underline">Cerrar</button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {data.unitNames.map((name, index) => (
                    <div key={index} className="space-y-1">
                      <label className="text-[9px] font-bold text-slate-400 uppercase">Unidad {index + 1}</label>
                      <input 
                        type="text"
                        value={name}
                        onChange={(e) => handleUnitNameChange(index, e.target.value)}
                        placeholder={`Título de la Unidad ${index + 1}`}
                        className="w-full bg-white border border-slate-200 p-3 rounded-lg text-xs font-bold outline-none focus:ring-2 focus:ring-red-600/20"
                      />
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Periodo / Año</label>
              <div className="flex gap-2">
                <select 
                  value={data.periodo}
                ...
                <select 
                  ...
                  className="w-24 bg-slate-50 border border-slate-200 p-4 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-red-600/20 text-center appearance-none"
                >
          ...
          <div className="space-y-8">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-black text-slate-900 uppercase flex items-center gap-3">
                <span className="w-6 h-6 bg-red-600 rounded-lg"></span>
              ...
              {currentAreaCurriculum.competencies.map(comp => (
                <div key={comp.id} className={`bg-white rounded-[2rem] p-8 border-2 transition-all duration-300 group/card ${
                  selectedCompetencies.includes(comp.id) 
                    ? 'border-red-500 shadow-xl shadow-red-500/5 ring-4 ring-red-500/5' 
                    : 'border-slate-100 hover:border-red-200 hover:shadow-lg'
                }`}>
                  <div className="flex justify-between items-start gap-4">
                    <label className="flex items-start gap-4 cursor-pointer group flex-1">
                      ...
                      <div className="space-y-1">
                        <span className={`text-sm font-black leading-tight block transition-colors ${
                          selectedCompetencies.includes(comp.id) ? 'text-slate-900' : 'text-slate-600 group-hover:text-red-600'
                        }`}>
                          {comp.name}
                        </span>
                        {!selectedCompetencies.includes(comp.id) && (
                    ...
                    {selectedCompetencies.includes(comp.id) && (
                      <button 
                        ...
                        className="shrink-0 text-[9px] font-black text-red-600 uppercase tracking-widest hover:bg-red-600 hover:text-white border border-red-200 px-3 py-1.5 rounded-full transition-all"
                      >
                        Todas
                      </button>
                    )}
                        ...
                        {comp.capacities.map(cap => (
                          <label key={cap.id} className={`flex items-start gap-3 cursor-pointer group/cap p-3 rounded-xl transition-all ${
                            ...
                            <input 
                              ...
                              className="mt-0.5 w-4 h-4 rounded border-slate-300 text-red-600 focus:ring-red-600"
                            />
          ...
          <div className="md:col-span-2 space-y-2">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Contexto / Diagnóstico Situacional</label>
            <textarea 
              value={data.contexto}
              onChange={(e) => setData({...data, contexto: e.target.value})}
              className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-red-600/20 min-h-[100px]"
            />
          </div>
        </div>
      </section>
...
export default AnnualPromptGenerator;
