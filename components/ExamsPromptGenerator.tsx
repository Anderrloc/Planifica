...
const DIFICULTADES = ["Básico", "Intermedio", "Avanzado", "Retador"];

const ExamsPromptGenerator: React.FC = () => {
  ...
  const generatePrompt = () => {
    ...
    return `Actúa como un Especialista en Evaluación y Elaboración de Reactivos del Ministerio de Educación (MINEDU) de Perú. 
Diseña un EXAMEN / EVALUACIÓN ESCRITA para el año escolar ${data.año} con un nivel de dificultad "${data.dificultad}".
...
II. PROPÓSITOS DE APRENDIZAJE EVALUADOS
Competencias y capacidades seleccionadas:
${compsText || "Seleccionar automáticamente según CNEB basándose en el tema de la sesión/unidad"}
...
El examen debe contener exactamente el siguiente número y tipo de preguntas:
- ${data.numOpcionMultiple} preguntas de Opción Múltiple (con 4 alternativas cada una).
- ${data.numVerdaderoFalso} preguntas de Verdadero o Falso.
- ${data.numRelacionar} preguntas de Relacionar columnas o conceptos.
- ${data.numCompletar} preguntas de Completar / Llenar espacios en blanco.
- ${data.numAbiertas} preguntas de Respuesta Abierta / Desarrollo (que evalúen pensamiento crítico).

IV. REQUERIMIENTOS TÉCNICOS
1. Presenta el examen listo para imprimir, con un encabezado profesional.
2. Las preguntas deben estar alineadas a los desempeños del grado y nivel de dificultad "${data.dificultad}".
3. Al final del examen, incluye el SOLUCIONARIO detallado con las respuestas correctas y los criterios de corrección para las preguntas abiertas.
4. Asegúrate de que el lenguaje sea claro y adecuado para la edad de los estudiantes.
    ...
    <div className="space-y-8 animate-fade-in pb-20">
      <section className="bg-white rounded-[2rem] shadow-xl border border-slate-200 overflow-hidden">
        <div className="bg-slate-900 text-white px-10 py-8">
          <h2 className="text-[12px] font-black text-rose-400 uppercase tracking-widest mb-2">Generador de Prompts</h2>
          <h3 className="text-2xl font-black uppercase">Exámenes / Evaluaciones</h3>
          <p className="text-slate-400 text-xs mt-2 font-bold uppercase tracking-widest">Configura los datos para obtener un prompt de examen profesional</p>
        </div>
        ...
        <div className="p-10 space-y-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input 
                ...
                className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-rose-600/20"
              />
            </div>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Grado de Dificultad</label>
              <select 
                value={data.dificultad}
                onChange={(e) => setData({...data, dificultad: e.target.value})}
                className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-rose-600/20 appearance-none"
              >
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <select 
                  ...
                  className="flex-1 bg-slate-50 border border-slate-200 p-4 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-rose-600/20 appearance-none"
                >
          ...
          <div className="space-y-6">
            <h4 className="text-sm font-black text-slate-900 uppercase flex items-center gap-3">
              <span className="w-6 h-6 bg-rose-600 rounded-lg"></span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 bg-slate-50 p-6 rounded-2xl border border-slate-200">
              <div className="space-y-2">
                ...
                <input 
                  type="number" 
                  value={data.numOpcionMultiple}
                  onChange={(e) => setData({...data, numOpcionMultiple: parseInt(e.target.value)})}
              ...
              <div className="space-y-2">
                ...
                <input 
                  type="number" 
                  value={data.numVerdaderoFalso}
                  onChange={(e) => setData({...data, numVerdaderoFalso: parseInt(e.target.value)})}
                  className="w-full bg-white border border-slate-200 p-3 rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-rose-600/20"
                />
              </div>
              <div className="space-y-2">
                ...
                <input 
                  type="number" 
                  value={data.numRelacionar}
                  onChange={(e) => setData({...data, numRelacionar: parseInt(e.target.value)})}
                  className="w-full bg-white border border-slate-200 p-3 rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-rose-600/20"
                />
              </div>
              <div className="space-y-2">
                ...
                <input 
                  type="number" 
                  value={data.numCompletar}
                  onChange={(e) => setData({...data, numCompletar: parseInt(e.target.value)})}
                  className="w-full bg-white border border-slate-200 p-3 rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-rose-600/20"
                />
              </div>
              <div className="space-y-2">
                <label className="text-[9px] font-black text-slate-500 uppercase">Abiertas / Desarrollo</label>
                <input 
                  type="number" 
                  value={data.numAbiertas}
                  onChange={(e) => setData({...data, numAbiertas: parseInt(e.target.value)})}
                  className="w-full bg-white border border-slate-200 p-3 rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-rose-600/20"
                />
              </div>
            </div>
              ...
              {currentAreaCurriculum.competencies.map(comp => (
                <div key={comp.id} className={`bg-white rounded-[2rem] p-8 border-2 transition-all duration-300 group/card ${
                  selectedCompetencies.includes(comp.id) 
                    ? 'border-rose-500 shadow-xl shadow-rose-500/5 ring-4 ring-rose-500/5' 
                    : 'border-slate-100 hover:border-rose-200 hover:shadow-lg'
                }`}>
                  <div className="flex justify-between items-start gap-4">
                    <label className="flex items-start gap-4 cursor-pointer group flex-1">
                      ...
                      <div className="space-y-1">
                        <span className={`text-sm font-black leading-tight block transition-colors ${
                          selectedCompetencies.includes(comp.id) ? 'text-slate-900' : 'text-slate-600 group-hover:text-rose-600'
                        }`}>
                          {comp.name}
                        </span>
                        {!selectedCompetencies.includes(comp.id) && (
                    ...
                    {selectedCompetencies.includes(comp.id) && (
                      <button 
                        ...
                        className="shrink-0 text-[9px] font-black text-rose-600 uppercase tracking-widest hover:bg-rose-600 hover:text-white border border-rose-200 px-3 py-1.5 rounded-full transition-all"
                      >
                        Todas
                      </button>
                    )}
                ...
                <div key={comp.id} className="bg-slate-50 rounded-2xl p-6 border border-slate-200">
                  <div className="flex justify-between items-center mb-4">
                    <label className="flex items-center gap-3 cursor-pointer group">
                      ...
                      />
                      <span className="text-sm font-black text-slate-800 group-hover:text-rose-600 transition-colors">{comp.name}</span>
                    </label>
                    
                    {selectedTransversalCompetencies.includes(comp.id) && (
                      <button 
                        onClick={() => selectAllTransversalCapacities(comp.id)}
                        className="text-[9px] font-black text-rose-600 uppercase tracking-widest hover:bg-rose-600 hover:text-white border border-rose-200 px-3 py-1.5 rounded-full transition-all"
                      >
                        Todas
                      </button>
                    )}
      ...
      <section className="bg-slate-900 rounded-[2rem] shadow-2xl overflow-hidden border border-slate-800">
        <div className="px-10 py-6 bg-slate-800/50 flex justify-between items-center">
          <h3 className="text-[10px] font-black text-rose-400 uppercase tracking-widest">Prompt Generado para Copiar</h3>
          <button 
            onClick={handleCopy}
            className={`flex items-center gap-2 px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${
              copied ? 'bg-emerald-500 text-white' : 'bg-rose-600 text-white hover:bg-rose-500'
            }`}
          >
            {copied ? (
              <>
        ...
        <div className="p-10">
          <pre className="bg-slate-950 p-8 rounded-2xl text-rose-200 text-xs font-mono leading-relaxed whitespace-pre-wrap border border-slate-800">
            {generatePrompt()}
          </pre>
        </div>
      </section>
...
export default ExamsPromptGenerator;
